...
int contarMinPaginas(NodeB *tree) {
    int i, count = 0, minChaves = ((N - 1) / 2);
    
    if (tree == NULL) {
        return 0;
    }

    if (tree->nro_chaves == minChaves) {
        count = 1;
    }

    for (i = 0; i <= tree->nro_chaves; i++) {
        count += contarMinPaginas(tree->filhos[i]);
    }
...
int main() {
    ...
    int numMinPaginas = contarMinPaginas(tree);
    printf("%d\n", numMinPaginas);
