...
void impNaoFolhaPreOrdem(NodeAVL* tree) {
    if (tree == NULL)
        return;
    if (tree->left != NULL || tree->right != NULL)
        printf("%d ", tree->item);
    impNaoFolhaPreOrdem(tree->left);
    impNaoFolhaPreOrdem(tree->right);
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        int valor;
        scanf("%d", &valor);
        raiz = inserirAVL(raiz, valor, &grown);
    }
    impNaoFolhaPreOrdem(raiz);
    printf("\n");
    return 0;
}
