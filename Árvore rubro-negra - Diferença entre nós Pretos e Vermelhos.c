...
int somatorioNos(NodeVP* tree) {
    ...
    if (tree->color == 1) {
        ...
        somaVermelhos += tree->key;
    }
    
    somaPretos += somatorioNos(tree->left);
    somaPretos += somatorioNos(tree->right);

    return somaPretos - somaVermelhos;
}

int main() {
    int i, m, item, diferenca = 0;
    NodeVP* tree = NULL;
